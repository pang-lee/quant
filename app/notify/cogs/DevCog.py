...
class DevCog(commands.Cog):
    ...
    def create_msg_embed(self, title, description, footer, params, color=0x00ff00):
        ...
        embed = Embed(
            title=title,
            description=f'測試模擬盤「{description}」',
            color=color,
            timestamp=datetime.now(pytz.timezone(self.timezone))  # 使用指定時區
        )
        
        embed.set_author(name="測試與模擬盤通知")
    ...
    async def send_dev_notification(self, title, description, footer, params, color=0x00ff00):
...
async def setup(bot):
    await bot.add_cog(DevCog(bot, int(os.getenv('DISCORD_DEV_CHANNEL'))))
