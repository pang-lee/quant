...
class OrderCog(commands.Cog):
    ...
    def create_order_embed(self, title, description, footer, params, color=0xD3D3D3):
        """建立一個股票通知的 embed，並添加動態欄位"""
        embed = Embed(
            title=title,
            description=f'以下是Order指令「{description}」',
            color=color,
            timestamp=datetime.now(pytz.timezone(self.timezone))  # 使用指定時區
        )
        
        embed.set_author(name="Order訂單帳戶操作")
    ...
    def create_msg_embed(self, title, description, footer, params, color=0x00ff00):
        ...
        embed = Embed(
            title=title,
            description=f'以下是最新的進場資訊「{description}」',
            color=color,
            timestamp=datetime.now(pytz.timezone(self.timezone))  # 使用指定時區
        )
    ...
    async def send_order_notification(self, title, description, footer, params, color=0x00ff00):
        """推播股票下單訊息到指定頻道"""
        ...
        try:
            ...
            print(f"推播Order訊息時出現錯誤: {e}")

async def setup(bot):
    await bot.add_cog(OrderCog(bot, int(os.getenv('DISCORD_ORDER_CHANNEL'))))
